...
interface StaffFormState {
    ...
    s_phone: string;
    s_address: string;
    s_gender: string; // Use string for input, then convert to 'M' | 'F' | 'O' for API
}

// NEW INTERFACE: For the data sent to the API
interface StaffApiPayload {
    s_id?: string; // Optional for POST, required for PUT
    s_name: string;
    s_phone: string;
    s_address: string;
    s_gender: 'M' | 'F' | 'O'; // Enforce strict gender type for API
}
...
const StaffTable: React.FC<StaffTableProps> = ({
    ...
    const fetchStaffMembers = useCallback(async () => {
        ...
        try {
            ...
            let errorMessage = 'An unexpected error occurred while fetching staff members.';
    ...
    const handleAddEditSubmit = async (e: React.FormEvent) => {
        ...
        // Prepare payload with correct types for API
        const payload: StaffApiPayload = {
            s_name: formState.s_name,
            s_phone: formState.s_phone,
            s_address: formState.s_address,
            s_gender: formState.s_gender.toUpperCase() as 'M' | 'F' | 'O', // Cast to union type
        };

        // Basic validation for gender
        if (!['M', 'F', 'O'].includes(payload.s_gender)) {
            setError('Invalid gender. Must be "M", "F", or "O".');
            setLoading(false);
            return;
        }
        ...
        const url = '/api/staff';
        let body: StaffApiPayload; // Use the new API Payload type
        ...
        if (method === 'PUT') {
            ...
            body = { ...payload, s_id: currentStaffMember.s_id };
        } else {
            // For POST, s_id is auto-generated by the backend
            body = payload;
        }
        ...
        try {
            ...
            let errorMessage = `An unexpected error occurred while trying to ${method === 'POST' ? 'add' : 'update'} the staff member.`;
            if (err instanceof Error) {
                errorMessage = err.message;
            } else if (typeof err === 'string') {
                errorMessage = err;
    ...
    const handleDeleteStaffMember = async () => {
        ...
        try {
            ...
            let errorMessage = 'An unexpected error occurred while deleting the staff member.';
            if (err instanceof Error) {
                errorMessage = err.message;
            } else if (typeof err === 'string') {
                errorMessage = err;
